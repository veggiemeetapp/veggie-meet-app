...
/**
 * Auth-only routes are never valid destinations after a successful sign-in.
 * Keeping this decision in one place also prevents a stale `next` value from
 * sending a valid member back through the sign-in surface.
 */
export function resolvePostAuthDestination(raw: string | null | undefined): string {
  const safe = sanitizeInternalPath(raw);
  if (!safe) return "/";
...
/** Provider callbacks can report an explicit failure in the query or hash. */
