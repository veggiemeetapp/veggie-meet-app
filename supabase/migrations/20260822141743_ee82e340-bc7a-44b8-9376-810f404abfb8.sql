-- WO-127 — shared eligible-upcoming-Meetup definition for Today.
-- Root cause: get_my_today_experience() hard-filtered Today's Upcoming Meetups with
--   AND m.host_id <> me                (hosted Meetups were invisible)
--   AND m.id <> ALL(used_meetup_ids)   (the check-in / primary-action Meetup was removed)
-- and required free capacity even for the host.

CREATE OR REPLACE FUNCTION public.eligible_upcoming_meetups_for_viewer(_me uuid, _city_id uuid)
RETURNS TABLE (
  id uuid,
  title text,
  category public.meetup_category,
  date date,
  ...
  host_id uuid,
  capacity integer,
  primary_interest_id text,
  additional_interest_ids text[],
  meetup_city_id uuid,
...
AS $$
  SELECT m.id, m.title, m.category, m.date, m.start_time, m.end_time,
         m.cover_image_url, m.host_id, m.capacity,
         m.primary_interest_id, m.additional_interest_ids, m.city_id,
         public.meetup_start_at(m.date, m.start_time, m.timezone),
         public.meetup_end_at(m.date, m.start_time, m.end_time, m.timezone),
         (SELECT COUNT(*) FROM attendance a WHERE a.meetup_id = m.id AND a.status IN ('joined','checked_in','attended')),
         EXISTS (SELECT 1 FROM attendance a WHERE a.meetup_id = m.id AND a.profile_id = _me AND a.status IN ('joined','checked_in','attended')),
         EXISTS (SELECT 1 FROM attendance a WHERE a.meetup_id = m.id AND a.profile_id = _me AND a.status = 'removed'),
         (m.host_id = _me)
  FROM meetups m
  ...
  WHERE m.status <> 'cancelled'::meetup_status
    ...
    AND NOT public.is_blocked_between(_me, m.host_id)
    AND (_city_id IS NULL OR m.city_id IS NULL OR m.city_id = _city_id)
$$;

REVOKE ALL ON FUNCTION public.eligible_upcoming_meetups_for_viewer(uuid, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.eligible_upcoming_meetups_for_viewer(uuid, uuid) FROM anon;
REVOKE ALL ON FUNCTION public.eligible_upcoming_meetups_for_viewer(uuid, uuid) FROM authenticated;
...
BEGIN
  ...
  -- Meetup recommendations — WO-127.
  -- Eligibility comes from the shared definition. Hosting, attendance and
  -- primary-action selection only affect ranking / labels, never eligibility.
  WITH hidden AS (
  ...
  eligible AS (
    SELECT e.*,
      (CASE WHEN e.start_at < now_ts + interval '24 hours' THEN 100
            WHEN e.start_at < now_ts + interval '48 hours' THEN 60
            WHEN e.start_at < now_ts + interval '7 days' THEN 40
            ELSE 20 END)
      + (CASE WHEN my_city_id IS NOT NULL AND e.meetup_city_id = my_city_id THEN 30 ELSE 0 END)
      + (CASE WHEN e.is_host OR e.is_attending THEN 25 ELSE 0 END)
      + (CASE WHEN e.category::text = ANY(COALESCE(my_interests, ARRAY[]::text[])) THEN 20 ELSE 0 END)
      + public.meetup_interest_score(e.primary_interest_id, e.additional_interest_ids, my_interests) AS score
    FROM public.eligible_upcoming_meetups_for_viewer(me, my_city_id) e
    WHERE e.id::text NOT IN (SELECT entity_id FROM hidden)
  ),
  final AS (
    SELECT e.*,
      CASE
        WHEN e.is_host THEN 'youre_hosting'
        WHEN e.start_at <= (now_ts + interval '90 minutes') THEN 'starts_soon'
        WHEN e.start_at < (now_ts + interval '24 hours') THEN 'happening_today'
        WHEN public.meetup_interest_score(e.primary_interest_id, e.additional_interest_ids, my_interests) > 0 OR e.category::text = ANY(COALESCE(my_interests, ARRAY[]::text[])) THEN 'matches_your_interests'
        WHEN my_city_id IS NOT NULL AND e.meetup_city_id = my_city_id THEN 'in_your_city'
        ELSE 'new_meetup'
      END AS reason_code_calc
    FROM eligible e
    WHERE (e.is_host OR e.is_attending OR e.capacity > e.attendee_count) AND NOT e.is_removed
  )
  ...
  FROM (
    SELECT f.id, f.score, f.start_at,
      jsonb_build_object(
        ...
        'title', f.title, 'category', f.category, 'primary_interest_id', f.primary_interest_id, 'additional_interest_ids', COALESCE(f.additional_interest_ids, ARRAY[]::text[]),
        ...
        'reason_label', CASE f.reason_code_calc
          WHEN 'youre_hosting' THEN 'You’re hosting'
          ...
          ELSE 'New meetup'
        END,
        'action_type', CASE WHEN f.is_attending OR f.is_host THEN 'view_meetup' ELSE 'join_meetup' END
      ) AS row_json
    FROM final f
    ORDER BY score DESC, start_at, id LIMIT 5
  ) t;
