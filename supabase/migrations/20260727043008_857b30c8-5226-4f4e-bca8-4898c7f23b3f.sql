...
BEGIN
  ...
  SELECT
    COALESCE(jsonb_agg(public.to_plan(f.*) ORDER BY f.starts_at) FILTER (
      WHERE f.plan_type IN ('active','update','invitation','cancelled','follow_up')
    ), '[]'::jsonb),
    COALESCE(jsonb_agg(public.to_plan(f.*) ORDER BY f.starts_at) FILTER (
      WHERE f.plan_type = 'upcoming'
    ), '[]'::jsonb),
    COALESCE(jsonb_agg(public.to_plan(f.*) ORDER BY f.starts_at) FILTER (
      WHERE f.plan_type = 'hosting'
    ), '[]'::jsonb)
  INTO needs_attention, upcoming, hosting
  FROM final f;

  WITH pastq AS (
    SELECT c.* FROM classified c
    WHERE c.plan_type = 'past'
      AND (_past_cursor IS NULL OR c.starts_at < _past_cursor)
    ORDER BY c.starts_at DESC
    LIMIT _past_limit
  )
  SELECT
    COALESCE(jsonb_agg(public.to_plan(p.*) ORDER BY p.starts_at DESC), '[]'::jsonb),
    MIN(p.starts_at)
  INTO past, past_next
  FROM pastq p;
...
REVOKE ALL ON FUNCTION public.get_my_plans(TIMESTAMPTZ, INT) FROM PUBLIC, anon;
