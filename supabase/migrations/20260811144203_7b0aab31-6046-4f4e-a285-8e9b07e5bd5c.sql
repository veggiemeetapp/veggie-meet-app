...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'stages', jsonb_build_object(
      'joined', (SELECT coalesce(max(members),0) FROM per_event WHERE event_name='onboarding_completed'),
      'reached_today', (SELECT coalesce(max(members),0) FROM per_event WHERE event_name='today_opened'),
      'explored_community', (SELECT count(*)::int FROM explored),
      'viewed_place', (SELECT coalesce(max(members),0) FROM per_event WHERE event_name='community_place_detail_opened'),
      'opened_host', (SELECT coalesce(max(members),0) FROM per_event WHERE event_name='host_opened'),
