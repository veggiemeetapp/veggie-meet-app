...
AS $function$
DECLARE me uuid; m RECORD; city RECORD;
...
BEGIN
  ...
  VALUES (_meetup_id, me, m.city_id, _city_id, m.location_name, _location_name,
    ...
    m.timezone, _timezone, meaningful) RETURNING id INTO change_id;
  ...
  IF meaningful THEN
    ...
    WITH eligible_recipients AS (
      SELECT DISTINCT profile_id FROM (
        ...
        UNION
        SELECT mi.recipient_id AS profile_id
          FROM public.meetup_invitations mi
         WHERE mi.meetup_id = _meetup_id
           AND mi.recipient_id <> me
           AND mi.status::text IN ('invited','viewed')
      ) r WHERE profile_id IS NOT NULL
    ),
    counted AS (
      SELECT count(*) AS n FROM eligible_recipients
    ),
    inserted AS (
      INSERT INTO public.notifications (
        recipient_id, actor_id, type, entity_type, entity_id,
        destination_type, destination_id, title, body, metadata, dedup_key)
      SELECT er.profile_id, me, 'meetup_updated'::notification_type,
        ...
        jsonb_build_object(
          ...
          'new_city', city.name, 'new_location_name', _location_name,
  ...
  RETURN jsonb_build_object(
    'meetup_id', _meetup_id, 'change_id', change_id,
    'meaningful_change', meaningful, 'coord_shift_m', coord_shift_m,
